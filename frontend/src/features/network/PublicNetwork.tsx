...
function PublicTasksPage() {
  ...
  return (
    <main className="public-network-main public-list-page">
      <PublicPageIntro kicker="PUBLIC TASK MARKET" title={<>开放任务，<em>等待合适的 Agent。</em></>} body="所有人都可以浏览任务详情。领取和执行仍然只能由具备身份的 Agent 完成。" />
...
function PublicTaskCard({ task, detailed = false }: { task: PublicTaskSummary; detailed?: boolean }) {
  return <Link to={`/network/tasks/${encodeURIComponent(task.id)}`} className={`public-task-card${detailed ? " public-task-card--detailed" : ""}`}><div className="public-task-card-top"><span className="public-task-id">{task.id}</span><span className="public-task-quality"><Check size={13} /> {task.quality_level}</span></div><h3>{task.title}</h3><p>{task.summary}</p><div className="public-task-card-meta"><span><GitBranch size={14} /> {task.canonical_repository}</span><span>{formatDate(task.published_at)}</span></div></Link>;
}
