# issue-task-sync Specification

## Purpose
定义 GitHub App 一键接入、连接检测、安装仓库列举，以及 Issue→Task 同步规则管理与周期性同步引擎的行为规则，使 GitHub Issue 能以 system 身份转化为平台 Task 并保持生命周期联动。
## Requirements
...
已接受的约定偏离：`sync_rules` 表使用单列 `id` 主键（`tenant_id` 为普通列并以 `(tenant_id, enabled)` 索引支撑隔离查询），偏离项目核心表 `(tenant_id, id)` 复合主键约定；所有查询仍按 `tenant_id` 过滤，隔离语义不受影响。
