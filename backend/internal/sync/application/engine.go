...
type IssueSourceProvider interface {
	IssueSource(ctx context.Context, tenantID, repo, sourceAuth string) (git.IssueSource, error)
}
...
func (e *Engine) RunRule(ctx context.Context, tenantID, ruleID string) (SyncResult, error) {
	...
	source, err := e.sources.IssueSource(ctx, tenantID, rule.Repo, rule.SourceAuth)
	if err != nil {
		return result, err
	}
	now := e.opts.Now()
