...
func TestNewExperienceCandidateRequiresRequiredFields(t *testing.T) {
	now := time.Now()
	_, err := NewExperienceCandidate("", "tenant", "agent", "task", "sub", "rev", "evidence", nil, "public", now)
...
func TestCandidateAutoRejectForbidden(t *testing.T) {
	...
	require.NoError(t, c2.ClassifyAndApply(policy2))
	require.Equal(t, StatusPendingReview, c2.StatusValue())

	// forbidden/pending_review cannot approve
	cForbidden := NewTestCandidate(t)
	_ = cForbidden.ClassifyAndApply(MockSensitivityPolicy{Class: SensitivityForbidden, Reason: "secret"})
	...
	_ = c3.ClassifyAndApply(MockSensitivityPolicy{Class: SensitivityInternal})
	...
	_ = c4.ClassifyAndApply(MockSensitivityPolicy{Class: SensitivityInternal})
...
func TestCandidateRejectRecordsReason(t *testing.T) {
	c := NewTestCandidate(t)
	_ = c.ClassifyAndApply(MockSensitivityPolicy{Class: SensitivityInternal})
